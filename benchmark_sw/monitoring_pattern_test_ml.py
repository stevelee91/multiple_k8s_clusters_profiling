...
if __name__ == '__main__':
    ...
    Cluster_object[0].Master_ip = '210.125.84.128'
    #Cluster_object[0].Master_ip = ''
    #Cluster_object[0].Worker_ip.append('')
    Cluster_object[0].Worker_ip.append('210.125.84.122')
    #Cluster_object[0].Worker_ip.append('210.125.84.24')
    #Cluster_object[0].Worker_ip.append('210.125.84.25')
    #Cluster_object[0].Worker_ip.append('210.125.84.26')
    ...
    for i in range(1, int(Deployment_cnt)+1):
        ...
        #################### computing part how many number of nodes in the cluster
            ...
            #storing initial value for cluster
            for i in range(0, int(cluster_num)):
            ...
            if Type_of_pattern == '1':
                ...
                db = conn.C1_Web_App_DB
            elif Type_of_pattern == '3':
                pattern_type = 'IoTCloud_%s_' % cnt_input
                db = conn.C1_IoT_Cloud
    ...
    ########################################################################################################################
            ...
            for i in range(0, int(cluster_num)):
                if Cluster_object[i].Circulation_cnt > 1:
                    if float(Now_avg_cpu_usage) - (float(Initial_avg_cpu_usage) * 0.05) <= Initial_avg_cpu_usage:
