...
for i in range(1, device_cnt):
    consumer = KafkaConsumer('Device'+ str(i), bootstrap_servers=['kafka-broker-1:9092'])
    ...
    while partitions == None or len(partitions) == 0:
        ...
        cmd = "curl -XPOST 'influxdb:8086/write?db=Sensordata' --data-binary 'dev%s,location=Gwangju temperature=%s,humidity=%s,Aircon_temp=%s,ext_temperature=%s,ext_humidity=%s,weather_stat=%s'" % (i ,temperature, humidity, Aircon_temp, ext_temperature, ext_humidity, weather_stat)
        subprocess.call([cmd], shell=True)
