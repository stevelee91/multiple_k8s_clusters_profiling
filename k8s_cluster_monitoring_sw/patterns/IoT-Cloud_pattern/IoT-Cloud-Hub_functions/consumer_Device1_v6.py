...
while partitions == None or len(partitions) == 0:
        ...
        cmd = "curl -XPOST 'influxdb:8086/write?db=Sensordata' --data-binary '%s,location=Gwangju temperature=%s,humidity=%s,Aircon_temp=%s,ext_temperature=%s,ext_humidity=%s,weather_stat=%s'" % (device_name, temperature, humidity, Aircon_temp, ext_temperature, ext_humidity, weather_stat)
        subprocess.call([cmd], shell=True)
